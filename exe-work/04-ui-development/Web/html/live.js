...
$(function() {
	...
	function GetCorridorMode(callback){
	...
	function CheckFlipMirrorAfterPreview(ch, callback){
        RfParamCall(function(a){
            if (a.Ret == 100) {
                if(bIPC){
                    $("#Mirror").prop("checked", a[a.Name].MirrorOperation);
                    $("#Flip").prop("checked", a[a.Name].FlipOperation);
                }else{
                    $("#Mirror").prop("checked", a[a.Name][0].MirrorOperation);
                    $("#Flip").prop("checked", a[a.Name][0].FlipOperation);
                }
            }else{
            ...
            callback();
        }, pageTitle, "Uart.PTZControlCmd", ch, WSMsgID.WsMsgID_CONFIG_GET);
	}
	function PreviewPlayCallback(a){
		...
		if(a.Ret == WEB_ERROR.ERR_SUCESS){
            ...
            CheckFlipMirrorAfterPreview(a.Chn, function(){
                GetCorridorMode(function(){
                    MasklayerHide();
            ...
            if(bIPC){
                CheckFlipMirrorAfterPreview(0, function(){
                    MasklayerHide();
                    gDevice.runAutoCheckWnd();
                });
            }
	...
	$(function(){
					...
					if(b.PlayStatus == PlayStatus.StatusPlaying){
                        ...
                        if(gVar.CurChannel >= 0){
                            RfParamCall(function(a){
                                ...
                                MasklayerHide();
                            }, pageTitle, "Uart.PTZControlCmd", gVar.CurChannel, WSMsgID.WsMsgID_CONFIG_GET);
                        }
					}else{
			...
			if(id != "Stream_Stop"){
				...
				if(bIPC && GetFunAbility(gDevice.Ability.PreviewFunction.PreviewShowPedRule)){
					GetHumanCfg(0, HumanCfgCallBack, function(){
						gDevice.PreviewPlay(-1, Stream, c);
					});
				}
				else{
					gDevice.PreviewPlay(-1, Stream, c);
				...
				MasklayerShow();
				gDevice.PreviewStop(-1,function(a){
                    MasklayerHide();
                });
			}	
		});
		...
		$("#Mirror,#Flip").click(function(){
            var ch = bIPC && gDevice.loginRsp.ChannelNum == 1?0:gVar.CurChannel;
            if(ch < 0) {
                $("#Mirror").prop("checked", false);
                $("#Flip").prop("checked", false);
			...
			var cfg = {};
			cfg.Name = "Uart.PTZControlCmd.[" + ch + "]";
            if(!bIPC){
                cfg[cfg.Name]=[{"MirrorOperation":bMirror,"FlipOperation": bFlip,"ModifyCfg":true}];
            }else{
                cfg[cfg.Name]={"MirrorOperation":bMirror,"FlipOperation": bFlip,"ModifyCfg":true};
            }
			RfParamCall(function(b){
			...
			}, pageTitle, "Uart.PTZControlCmd", ch, WSMsgID.WsMsgID_CONFIG_SET, cfg);
		});
