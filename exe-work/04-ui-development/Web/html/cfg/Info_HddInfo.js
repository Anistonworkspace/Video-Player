		...
		if (hddInfoArr != null) {
			for (var i=0; i < hddInfoArr.length; i++) {
				...
				for (var j=0; j < hddInfoArr[i].PartNumber; j++) {
					...
					if (supportPercent) {
                    tempData.Capacity = "100%";
                    tempData.LeftCapacity = getRemainPercentStr(leftCapacity, capacity, 2);
                } else {
                    tempData.Capacity = ChangeSpaceToString(capacity);
                    tempData.LeftCapacity = ChangeSpaceToString(leftCapacity);
                }
					listData.push(tempData);
			...
			if (supportPercent) {
    			listData[0].Capacity = "100%";
    			listData[0].LeftCapacity = getRemainPercentStr(allLeftCapacity, allCapacity, 2);
			} else {
