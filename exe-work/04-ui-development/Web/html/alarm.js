...
$(function() {
	...
	function FillAlarmInfo(aB){
		...
		switch (type){
			...
			case "HumanFallDown":
				...
				break;
            case "LeavePostDetect":
                if($("#LeavePostDetection_check").prop("checked")){
                    typeTitle = "Leave Post Detect";
                }
                break;
			default:
				typeTitle = "";
	...
	function SendAlarmTypeChecked(){
        ...
        var LeavePostDetect = $("#LeavePostDetection_check").prop("checked");
		var cmd ={
			'MainType': 14,
			'SubType': 0,
			'alarmtype':{
				...
				'Carshape': Carshape,
				'Falldown': Falldown,
                'LeavePostDetect':LeavePostDetect
			}
		}
	...
	function getAlarmType(){
        gDevice.getAlarmType(function(a){
			if(a.Ret == WEB_ERROR.ERR_SUCESS){
				gDevice.AlarmType = a;
				if(isObject(gDevice.AlarmType.alarmtype)){
                    ...
                    $("#LeavePostDetection_check").prop("checked", alarmType.LeavePostDetect);
                }	
			}
            MasklayerHide();
		});
	...
	$(function() {
        ...
        if(!GetFunAbility(gDevice.Ability.OtherFunction.LeavePostDetection)){
            $("#LeavePostDetection_box").css("display", "none");
        }
